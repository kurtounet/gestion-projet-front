...
export class ProjectInstantStore {
  ...
  readonly selectedSprint = signal<number | null>(null);
  ...
  currentSprints= signal<ISprintInstance[]>([]) ;
 ...
 this.resetTasks();
  this.projectInstanceService.getProjectInstanceById(id).subscribe({
    next: (data) => {
      ...
      this.getSprintCurrentProject(data.id);
...
getCurrentProjectTask(id:number){
  this.taskInstanceService.getAllTaskBySprintInstance(id).subscribe(data => this.currentTasks.set(data));
